import { LibraryImage, PlacedPhoto, Viewport } from '../store/types'
...
export function getPhotoSize(
  photo: Pick<PlacedPhoto, 'libraryImageId' | 'scale'>,
  libraryImages: Pick<LibraryImage, 'id' | 'width' | 'height'>[]
): { width: number; height: number } {
  const image = libraryImages.find((img) => img.id === photo.libraryImageId)
  const ratio = image?.width && image?.height ? image.height / image.width : 0.67
  const width = PHOTO_W * photo.scale
