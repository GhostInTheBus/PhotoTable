...
describe('loadCanvasState', () => {
  ...
  it('returns saved state', async () => {
    ...
    const result = await loadCanvasState('proj-123')
    expect(result).toEqual({ schemaVersion: 1, ...state })
  })

  it('returns null for invalid saved state', async () => {
    mockDB.get.mockResolvedValue({ photos: [{ id: 'p1' }], clumps: [], clumpCounter: 0, viewport: { x: 0, y: 0, zoom: 1 } })
    const result = await loadCanvasState('proj-123')
    expect(result).toBeNull()
  })
})
