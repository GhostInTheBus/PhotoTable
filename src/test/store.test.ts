...
describe('canvas slice', () => {
  ...
  it('removePhotos removes a group as one undoable change', () => {
    useStore.getState().addPhoto({ id: 'p1', libraryImageId: 'img1', x: 0, y: 0, rotation: 0, scale: 1, zIndex: 1, clumpId: null })
    useStore.getState().addPhoto({ id: 'p2', libraryImageId: 'img2', x: 0, y: 0, rotation: 0, scale: 1, zIndex: 2, clumpId: null })
    useStore.getState().removePhotos(['p1', 'p2'])
    expect(useStore.getState().canvas.photos).toHaveLength(0)
    useStore.getState().undo()
    expect(useStore.getState().canvas.photos).toHaveLength(2)
  })
  ...
  it('undo and redo restore canvas changes', () => {
    useStore.getState().addPhoto({ id: 'p1', libraryImageId: 'img1', x: 0, y: 0, rotation: 0, scale: 1, zIndex: 1, clumpId: null })
    expect(useStore.getState().canvas.photos).toHaveLength(1)
    useStore.getState().undo()
    expect(useStore.getState().canvas.photos).toHaveLength(0)
  ...
  it('can capture one history point for a gesture with silent moves', () => {
    useStore.getState().addPhoto({ id: 'p1', libraryImageId: 'img1', x: 0, y: 0, rotation: 0, scale: 1, zIndex: 1, clumpId: null })
    useStore.getState().captureHistory()
    useStore.getState().movePhoto('p1', 100, 0, false)
    useStore.getState().movePhoto('p1', 200, 0, false)
    useStore.getState().undo()
    expect(useStore.getState().canvas.photos[0].x).toBe(0)
  })
})
...
describe('library slice', () => {
  ...
  it('relinks missing photos by filename hint', () => {
    useStore.getState().addFolder({
      id: 'f1',
      name: 'Relink',
      handle: null,
      images: [{ id: 'new-img', filename: 'photo.jpg', folderPath: 'Relink', fileHandle: null, thumbnailUrl: 'blob:x' }],
      loaded: true,
    })
    useStore.getState().addPhoto({
      id: 'p1',
      libraryImageId: 'missing-img',
    ...
    useStore.getState().relinkMissingPhotos()
    expect(useStore.getState().canvas.photos[0].libraryImageId).toBe('new-img')
  })
})
